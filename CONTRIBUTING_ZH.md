...
- **Python >= 3.14**(在 `pyproject.toml` 中指定)
...
```bash
pip install -r requirements.txt   # Python 依赖(含开发依赖)
npm install                        # Node.js 开发工具(用于格式检查)
```
...
| 命令                   | 说明                                               |
| :--------------------- | :------------------------------------------------- |
| `npm run format:fix`   | 自动修复所有格式问题(Python + Markdown + 全角字符) |
| `npm run format:check` | 检查所有格式问题                                   |
| `npm run test`         | 运行 pytest 并生成覆盖率报告                       |
| `npm run ci`           | 完整 CI 流程: 格式修复 -> 测试 -> 格式检查         |

#### 运行应用

```bash
python main.py           # 使用文件夹选择对话框启动
python main.py -p /path  # 指定工作区路径启动
```

#### 运行应用(打包后)

```bash
manualaid           # 使用文件夹选择对话框启动
manualaid -p /path  # 指定工作区路径启动
```
