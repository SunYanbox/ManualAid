...
def _get_base_command(workspace_path: str | None = None) -> list[str]:
    ...
    base_cmd = [executable] if is_frozen() else [executable, script]
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        if workspace_path:
            ...
            if not path_obj.exists():
                context.console.print(f"[bold red]错误: 路径不存在: {path_obj}[/bold red]")
                return CommandResult(success=False, message=f"Path not found: {workspace_path}")
            if not path_obj.is_dir():
                context.console.print(f"[bold red]错误: 路径不是目录: {path_obj}[/bold red]")
        ...
        try:
            ...
            # 显示手动启动帮助
            if is_frozen():
                manual_cmd = f'"{get_executable_path()}" -p "{workspace_path}"'
            else:
