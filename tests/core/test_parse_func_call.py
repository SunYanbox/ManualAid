...
class TestParseFuncCall:
    ...
    def test_basic_kwargs(self):
        """关键字参数 kwargs"""
        content = (
            '<func_call>{"func_name": "write", "kwargs": '
            + '{"file_path": "a.txt", "old_str": "x", "new_str": "y"}}</func_call>'
        )
        func_name, args, kwargs = parse_func_call(content)
    ...
    def test_escaped_quotes_in_string(self):
        ...
        func_name, args, _ = parse_func_call(content)
    ...
    def test_nested_func_call_tags(self):
        ...
        content = (
            '<func_call>{"func_name": "outer", "args": '
            + '["<func_call>{\\"func_name\\": \\"inner\\"}</func_call>"]}</func_call>'
        )
